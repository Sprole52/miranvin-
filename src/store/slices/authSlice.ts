...
export const loginWithStaticAdmin = createAsyncThunk(
  ...
  async ({ email, password }: { email: string; password: string }, { rejectWithValue, dispatch }) => {
    try {
      // Önce veritabanında admin var mı kontrol et
      await dispatch(checkDatabaseAdmins()).unwrap();
      
      // Eğer veritabanında admin varsa statik admin ile giriş yapılamaz
      const hasAdmins = await dispatch(checkDatabaseAdmins()).unwrap();
      if (hasAdmins) {
        throw new Error('Veritabanında admin bulunduğu için statik admin ile giriş yapılamaz.');
      }
